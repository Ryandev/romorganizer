...
void banner(void) {
    log("UNECM - Decoder for Error Code Modeler format v1.0\n"
        "Copyright (C) 2002 Neill Corlett\n\n"
    );
}
...
EXPORT int unecmify(int in_fd, int out_fd) {
    ...
    fseek(in, 0, SEEK_END);
    log("DEBUG: Input file length = %ld bytes\n", ftell(in));
    resetcounter(ftell(in));
    log("DEBUG: Seeking to start of input file\n");
    fseek(in, 0, SEEK_SET);
    if (
            (fgetc(in) != 'E') ||
            (fgetc(in) != 'C') ||
    ...
    log("DEBUG: Header found, starting decode loop\n");
    for (;;) {
        int c = fgetc(in);
        int bits = 5;
        if (c == EOF) goto uneof;
    ...
    if (
        ...
        log("EDC error (%08X, should be %02X%02X%02X%02X)\n",
                checkedc,
                sector[3],
                sector[2],
                sector[1],
    ...
    fclose(in);
    fclose(out);
    log("DEBUG: unecmify completed successfully\n");
    return 0;
    uneof:
    log("DEBUG: Unexpected EOF!\n");
    log("Unexpected EOF!\n");
    corrupt:
    log("DEBUG: Corrupt ECM file!\n");
    log("Corrupt ECM file!\n");
    if (in) fclose(in);
    if (out) fclose(out);
    return 1;
}
...
int main(int argc, char **argv) {
    ...
    */
    if((argc != 2) && (argc != 3) && (argc != 4)) {
      log("usage: %s [--cue] ecmfile [outputfile]\n", argv[0]);
      return 1;
    }
    /*
    ...
    */
    ...
    if (strlen(infilename) < 5) {
      log("filename '%s' is too short\n", infilename);
      return 1;
    }
    if (strcasecmp(infilename + strlen(infilename) - 4, ".ecm")) {
      log("filename must end in .ecm\n");
      return 1;
    }
    /*
    ...
    ** Decode
    */
    unecmify(fileno(fin), fileno(fout));
    /*
    ** Close everything
    */
    fclose(fout);
