...
extern "C" {
#endif
EXPORT int ecmify(int in_fd, int out_fd);
#ifdef __cplusplus
}
#endif
...
void banner(void) {
    log("ECM - Encoder for Error Code Modeler format v1.0\n"
        "Copyright (C) 2002 Neill Corlett\n\n"
    );
}
...
void setcounter_analyze(unsigned n) {
    if ((n >> 20) != (mycounter_analyze >> 20)) {
        ...
        if (!d) d = 1;
        log("Analyzing (%02d%%) Encoding (%02d%%)\r",
                (100 * a) / d, (100 * e) / d
        );
    }
    mycounter_analyze = n;
...
EXPORT int ecmify(int in_fd, int out_fd) {
    ...
    int dataavail = 0;
    int typetally[4];
    log("DEBUG: Seeking to end of input file\n");
    fseek(in, 0, SEEK_END);
    intotallength = ftell(in);
    log("DEBUG: Input file length = %d bytes\n", intotallength);
    ...
    typetally[3] = 0;
    /* Magic identifier */
    log("DEBUG: Writing magic identifier\n");
    fputc('E', out);
    fputc('C', out);
    fputc('M', out);
    fputc(0x00, out);
    for (;;) {
        if ((dataavail < 2352) && (dataavail < (intotallength - inbufferpos))) {
            ...
            if (willread) {
                ...
                dataavail += willread;
                log("DEBUG: Read %d bytes, total dataavail=%d\n", willread, dataavail);
            }
        }
    ...
    log("DEBUG: Writing end-of-records indicator\n");
    ...
    /* Show report */
    log("Literal bytes........... %10d\n", typetally[0]);
    log("Mode 1 sectors.......... %10d\n", typetally[1]);
    log("Mode 2 form 1 sectors... %10d\n", typetally[2]);
    log("Mode 2 form 2 sectors... %10d\n", typetally[3]);
    log("Encoded %d bytes -> %ld bytes\n", intotallength, ftell(out));
    log("Done.\n");
    log("DEBUG: Closing files\n");
    fclose(in);
    fclose(out);
    log("DEBUG: ecmify completed successfully\n");
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    */
    if ((argc != 2) && (argc != 3)) {
        log("usage: %s cdimagefile [ecmfile]\n", argv[0]);
        return 1;
    }
    infilename = argv[1];
    /*
