...
const MAP_EXTRACT_COMMANDS = {
    '7z': (filePath: string, outputDir: string) => $`7z x "${filePath}" -o"${outputDir}" -y`,
    '7za': (filePath: string, outputDir: string) => $`7za x "${filePath}" -o"${outputDir}" -y`,
    'p7zip': (filePath: string, outputDir: string) => $`p7zip -d "${filePath}" -o"${outputDir}"`,
} as const;
...
const MAP_COMPRESS_COMMANDS = {
    '7z': (filePath: string, contentsDirectory: string) => $`7z a "${filePath}" "${contentsDirectory}/*" -y`,
    '7za': (filePath: string, contentsDirectory: string) => $`7za a "${filePath}" "${contentsDirectory}/*" -y`,
    'p7zip': (filePath: string, contentsDirectory: string) => $`p7zip -a "${filePath}" "${contentsDirectory}/*"`,
} as const;

async function _getInstalledCommands(): Promise<(keyof typeof MAP_EXTRACT_COMMANDS)[]> {
...
async function _getInstalledVerifyCommands(): Promise<(keyof typeof MAP_VERIFY_COMMANDS)[]> {
    const commands: (keyof typeof MAP_VERIFY_COMMANDS)[] = [];
    for (const name of Object.keys(MAP_VERIFY_COMMANDS) as (keyof typeof MAP_VERIFY_COMMANDS)[]) {
        if (await doesCommandExist(name) && await isCommandExecutable(name)) {
            commands.push(name);
        }
    }
...
const ERROR_7Z_NOT_INSTALLED = new Error(`7z extraction failed. Please ensure 7-Zip is installed:\n- Windows: Download from https://7-zip.org/\n- macOS: brew install p7zip\n- Linux: sudo apt install p7zip-full`);
const ERROR_7Z_COMPRESS_NOT_INSTALLED = new Error(`7z compression failed. Please ensure 7-Zip is installed:\n- Windows: Download from https://7-zip.org/\n- macOS: brew install p7zip\n- Linux: sudo apt install p7zip-full`);
    ...
    async verify(): Promise<boolean> {
        ...
        try {
            ...
            try {
                ...
                log.warn(`✗ ${this.filePath} is corrupted or invalid: Verification failed with ${primaryCommand}`);
                return false;
            }
        } catch (error) {
