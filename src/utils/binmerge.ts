 ...
 */
export function parseCueContent(
    cueContent: string,
    basePath: string = '',
    ...
    for (const line of lines) {
        ...
        if (fileMatch && fileMatch[1]) {
            const thisPath = basePath
                ? `${basePath}/${fileMatch[1]}`
                : fileMatch[1];
            thisFile = {
                filename: thisPath,
                tracks: [],
        ...
        if (trackMatch && trackMatch[1] && trackMatch[2] && thisFile) {
            ...
            if (globalBlocksize === 2352) {
                /* Only update if using default */
        ...
        for (let i = files[0].tracks.length - 1; i >= 0; i--) {
            ...
            if (t && t.indexes.length > 0 && t.indexes[0]) {
                ...
                if (i > 0) {
                    ...
                    if (
                        prevTrack &&
                        prevTrack.indexes.length > 0 &&
                        prevTrack.indexes[0]
                    ) {
                        prevTrack.sectors =
                            nextOffset - prevTrack.indexes[0].file_offset;
                    }
                }
            }
        }
...
export function split(
    ...
    basePath: string = '',
    globalBlocksize: number = 2352
): { cueContent: string; file: BinFile } {
    log.info('[INFO]\tParsing CUE content...');
