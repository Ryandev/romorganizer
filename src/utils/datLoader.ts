...
async function loadDatFromZip(zipPath: string): Promise<Dat> {
    ...
    try {
        ...
        const datFiles = allFiles.filter(file => 
            path.extname(file).toLowerCase() === '.dat'
        );
        ...
        if (datFiles.length > 1) {
            log.warn(`Multiple .dat files found in zip: ${datFiles.join(', ')}`);
            log.warn(`Using the first one: ${datFiles[0]}`);
        }
        
        const datFile = datFiles[0];
        if (!datFile) {
            throw new DatLoaderException(`No .dat files found in zip: ${zipPath}`);
        }
        ...
        log.info(`Successfully loaded DAT from zip: ${dat.system} with ${dat.games.length} games`);
...
export async function isZipWithDat(filePath: string): Promise<boolean> {
    ...
    try {
        ...
        const allFiles = await storage().list(extractedDir, { recursive: true });
        const hasDatFiles = allFiles.some(file => 
            path.extname(file).toLowerCase() === '.dat'
        );
