...
function parseDatFile(xmlContent: string): Dat {
    ...
    if (!parsed.datafile) {
        throw new DatParsingException(
            'Invalid DAT file: missing datafile root element'
        );
    }
    ...
    if (!datafile.header || !datafile.header.name) {
        throw new DatParsingException(
            'Invalid DAT file: missing header or header name'
        );
    }
    ...
    /* Parse games */
    if (datafile.game) {
        const games = Array.isArray(datafile.game)
            ? datafile.game
            : [datafile.game];
        ...
        for (const gameData of games) {
            if (!gameData['@_name']) {
                throw new DatParsingException(
                    'Game missing required name attribute'
                );
            }
            ...
            const game: Game = {
                ...
                ...(gameData.description && {
                    description:
                        gameData.description._text || gameData.description,
                }),
                ...(gameData.category && {
                    category: gameData.category._text || gameData.category,
                }),
            };
            dat.games.push(game);
            ...
            /* Parse ROMs */
            if (gameData.rom) {
                const roms = Array.isArray(gameData.rom)
                    ? gameData.rom
                    : [gameData.rom];
                ...
                for (const romData of roms) {
                    if (
                        ...
                        throw new DatParsingException(
                            'ROM missing required attributes (name, size, or sha1)'
                        );
                    }
                    ...
                    /* Add to SHA1 index */
                    const romsWithSha1 =
                        dat.romsBySha1hex.get(rom.sha1hex) || [];
 ...
 */
export function findGamesByCombinedBinSize(
    dat: Dat,
    combinedBinSize: number
): { matches: Game[]; status: 'match' | 'closest' | 'none' } {
    ...
    for (const game of dat.games) {
        ...
        if (combinedSize === combinedBinSize) {
            ...
            log.info(
                `Found candidate game "${game.name}" with combined size ${combinedSize} matching ${combinedBinSize}`
            );
            result.status = 'match';
            return result;
        }
    ...
    if (result.matches.length === 0) {
        ...
        for (const game of dat.games) {
            const combinedSize = game.roms.reduce(
                (sum, rom) => sum + rom.size,
                0
            );
            if (
                Math.abs(combinedSize - combinedBinSize) <
                Math.abs(closestMatchSize - combinedBinSize)
            ) {
                closestMatch = game;
        ...
        log.info(
            `Closest match "${closestMatch.name}" with size ${closestMatchSize} and size delta ${sizeDelta}`
        );
