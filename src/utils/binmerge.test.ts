...
describe('Binmerge Functions', () => {
    describe('Timestamp Conversion', () => {
        ...
        it('should throw error for invalid timestamp format', () => {
            expect(() => cueStampToSectors('invalid')).toThrow(
                'Invalid timestamp format: invalid'
            );
            expect(() => cueStampToSectors('1:2:3:4')).toThrow(
                'Invalid timestamp format: 1:2:3:4'
            );
            expect(() => cueStampToSectors('01:02')).toThrow(
                'Invalid timestamp format: 01:02'
            );
            expect(() => cueStampToSectors('')).toThrow(
                'Invalid timestamp format: '
            );
            expect(() => cueStampToSectors('abc:def:ghi')).toThrow(
                'Invalid timestamp format: abc:def:ghi'
            );
        });
    });
  ...
  TRACK 01 AUDIO
    INDEX 01 00:00:00`;
        ...
        it('should throw Error for empty CUE content', () => {
            expect(() => parseCueContent('')).toThrow(
                'Unable to parse any bin files in the cueSheet. Is it empty?'
            );
        });

        it('should throw Error for invalid CUE content', () => {
            expect(() => parseCueContent('INVALID CONTENT')).toThrow(
                'Unable to parse any bin files in the cueSheet. Is it empty?'
            );
        });
  ...
  TRACK 01 CDG
    INDEX 01 00:00:00`;
            ...
            expect(log.info).toHaveBeenCalledWith(
                expect.stringContaining('Locked blocksize to 2448')
            );
        });
...
FILE "track2.bin" BINARY
  ...
  TRACK 02 AUDIO
    INDEX 01 00:01:20`;
            ...
            expect(result.cueContent).toContain(
                'FILE "split (Track 1).bin" BINARY'
            );
            expect(result.cueContent).toContain(
                'FILE "split (Track 2).bin" BINARY'
            );
...
FILE "track2.bin" BINARY
  TRACK 02 AUDIO
    INDEX 01 00:00:00`;
            ...
            expect(() => split(cueContent, 'split')).toThrow(
                'Split operation requires exactly one input file'
            );
        });
  ...
  TRACK 01 AUDIO
    INDEX 01 invalid_timestamp`;
            ...
            expect(() => cueStampToSectors('invalid_timestamp')).toThrow(
