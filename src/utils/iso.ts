 ...
 */
async function convert(filePath: string, format: 'bin' | 'iso' | 'daa' = 'bin'): Promise<string> {
    guardFileExists(filePath, `Cannot extract ${filePath} as it does not exist`);
    ...
    try {
        ...
        const output = await withTimeout($`${command} convert "${filePath}" -o "${binPath}"`, DEFAULT_TIMEOUT_MS);
        guard(output.exitCode === 0, `Conversion failed for ${filePath}`);

        guardFileExists(binPath, `Bin file missing, does not exist: ${binPath}`);
        
        return binPath;
    } catch (error) {
        throw new Error(
            `Failed to convert ${filePath} to BIN/CUE: ${error}`
        );
    }
}
